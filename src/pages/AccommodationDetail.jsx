...
async function handleShareWA() {
  ...
  const text = 'Check out this property on Scalablenexus: *' + accommodation?.title + '* - $' + accommodation?.price + '\n' + url

  if (navigator.share) {
    try {
      const imageUrl = accommodation?.images?.length > 0 ? accommodation.images[0] : null
      if (imageUrl && navigator.canShare) {
        ...
        if (navigator.canShare({ files: [file] })) {
          await navigator.share({
            title: accommodation?.title || 'Property on Scalablenexus',
            text: (accommodation?.title || '') + ' - $' + accommodation?.price + '\n' + url,
            files: [file],
          })
          return
        }
      }
      await navigator.share({ title: accommodation?.title || '', text, url })
      return
    } catch (err) {

  ...
  return (
    <>
      <style>{`
        ...
        .edd-btn-wa {
          background: #25d366; color: white; border: none; padding: 7px 12px;
          border-radius: 20px; font-size: 12px; font-weight: 700; cursor: pointer;
          display: flex; align-items: center; gap: 5px; font-family: inherit;
        }
        .edd-btn-copy {
          background: #f1f5f9; color: #374151; border: none; padding: 7px 12px;
        ...
        .edd-similar-img {
          width: 70px; height: 52px; border-radius: 9px; object-fit: cover;
          flex-shrink: 0; background: #fdf2f8; display: flex; align-items: center; justify-content: center; font-size: 24px;
        }

        @media (min-width: 769px) {
          .edd-cta { position: static; box-shadow: none; background: transparent; padding: 0; margin-bottom: 16px; }
          .edd-title { font-size: 28px; }
        }
        ...
        <div className="edd-header">
          <div className="edd-header-inner">
            ...
            <div className="edd-meta">
              {event.location?.city && (
                <span className="edd-meta-item">📍 {event.location.city}{event.location.area ? ', ' + event.location.area : ''}</span>
              )}
            ...
            <div className="edd-card-body">
              <div className="edd-info-grid">
                {event.location?.country && (
                  <div className="edd-info-box">
                    <div className="edd-info-label">Country</div>
                    <div className="edd-info-value">🌍 {event.location.country}</div>
                  </div>
                )}
                {event.location?.city && (
                  <div className="edd-info-box">
                    <div className="edd-info-label">City</div>
                    <div className="edd-info-value">🏙️ {event.location.city}</div>
                  </div>
                )}
                {event.location?.area && (
                  <div className="edd-info-box">
                    <div className="edd-info-label">Area / Campus</div>
                    <div className="edd-info-value">📍 {event.location.area}</div>
                  </div>
                )}
                {eventDate && (
                  <div className="edd-info-box">
                    <div className="edd-info-label">Date</div>
                    <div className="edd-info-value">📅 {eventDate}</div>
                  </div>
                )}
              </div>
            </div>
              ...
              {similar.map(e => (
                <div key={e._id} className="edd-similar-card" onClick={() => navigate('/events/' + e._id)}>
                  <div className="edd-similar-img">
                    {e.images?.[0]
                      ? <img src={e.images[0]} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '9px' }} />
                      : '🎉'
                    }
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ margin: '0 0 3px', fontSize: '11.5px', color: '#9ca3af' }}>
                      📍 {e.location?.city}{e.location?.area ? ', ' + e.location.area : ''}
                    </p>
                    {e.date && (
                      <p style={{ margin: 0, fontSize: '11.5px', fontWeight: 700, color: '#be185d' }}>
